...
$filtros = [['grid' => 'col-6 col-sm-4', 'entidade' => 'Pais']];

// Campos apresentados na lista
$Ufs = [
            ['label' => Config::getAliasesField('siglaUf'), 'atributo' => 'siglaUf', 'class'=>'text-left'],

            ['label' => Config::getAliasesField('idPais'), 'atributo' => 'Pais.nomePais', 'class'=>'text-left'],

            ['label' => Config::getAliasesField('nomeUf'), 'atributo' => 'nomeUf', 'class'=>'text-left']
    ];
$Ufs[0]['ngclick'] = $onclick;

// cards
$card = Card::basic($Ufs, $entidade, false, $entidade.'ContextItens');

// table
foreach ($Ufs as $campo) {
...
// Form
$form = [
Form::getModel(Html::input(['ng-model' => 'Uf.siglaUf', 'type'=>'text', 'class'=>'', 'required'=>'required ', 'ng-change' => $ngChange], Config::getAliasesField('siglaUf')), 'col-sm-4'), 
Form::getModel(Html::inputSelectNgRepeat('Uf.idPais', Config::getAliasesField('idPais'), 'Uf.idPais_ro', 'Aux.Pais', $ngClick, $ngChange), 'col-sm-4'), 
Form::getModel(Html::input(['ng-model' => 'Uf.nomeUf', 'type'=>'text', 'class'=>'', 'required'=>'required ', 'ng-change' => $ngChange], Config::getAliasesField('nomeUf')), 'col-sm-4')
];
...
$tableFiltros = new Table(['',''], false, false, 'table-bordered', false);$tableFiltros->setExplode(false);$tableFiltros->addLinha([
                  '<p class="text-strong">Pais</p>
                  <p ng-repeat="filter in Aux.Pais | filter: {idPais:Args.idPais}:true">{{filter.nomePais}}</p>',
