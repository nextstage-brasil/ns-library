...
$filtros = [['grid' => 'col-6 col-sm-4', 'entidade' => 'GradePolo'],
['grid' => 'col-6 col-sm-4', 'entidade' => 'Usuario']];

// Campos apresentados na lista
$Forums = [
            ['label' => Config::getAliasesField('idGradePolo'), 'atributo' => 'GradePolo.nomeGradePolo', 'class'=>'text-left'],

            ['label' => Config::getAliasesField('nomeForum'), 'atributo' => 'nomeForum', 'class'=>'text-left'],

            ['label' => Config::getAliasesField('dataInicioForum'), 'atributo' => 'dataInicioForum', 'class'=>'text-left'],

            ['label' => Config::getAliasesField('dataFimForum'), 'atributo' => 'dataFimForum', 'class'=>'text-left'],

            ['label' => Config::getAliasesField('onlyGradeForum'), 'atributo' => 'onlyGradeForumF', 'class'=>'text-left'],

            ['label' => Config::getAliasesField('moderadaoresForum'), 'atributo' => 'moderadaoresForum', 'class'=>'text-left'],

            ['label' => Config::getAliasesField('idUsuario'), 'atributo' => 'Usuario.nomeUsuario', 'class'=>'text-left']
    ];
$Forums[0]['ngclick'] = $onclick;

// cards
$card = Card::basic($Forums, $entidade, false, $entidade.'ContextItens');

// table
foreach ($Forums as $campo) {
...
// Form
$form = [
Form::getModel(Html::inputSelectNgRepeat('Forum.idGradePolo', Config::getAliasesField('idGradePolo'), 'Forum.idGradePolo_ro', 'Aux.GradePolo', $ngClick, $ngChange), 'col-sm-4'), 
Form::getModel(Html::input(['ng-model' => 'Forum.nomeForum', 'type'=>'text', 'class'=>'', 'required'=>'required ', 'ng-change' => $ngChange], Config::getAliasesField('nomeForum')), 'col-sm-4'), 
Form::getModel(Html::inputDatePicker(Config::getAliasesField('dataInicioForum'), 'Forum.dataInicioForum', $minDate, $maxDate, $ngChange), 'col-sm-4'), 
Form::getModel(Html::inputDatePicker(Config::getAliasesField('dataFimForum'), 'Forum.dataFimForum', $minDate, $maxDate, $ngChange), 'col-sm-4'), 
Form::getModel(Html::inputSelectNgRepeat('Forum.onlyGradeForum', Config::getAliasesField('onlyGradeForum'), 'Forum.onlyGradeForum_ro', 'Aux.Boolean', $ngClick, $ngChange, 'Boolean'), 'col-sm-4'), 
Form::getModel('<config-json title="'.Config::getAliasesField('moderadaoresForum').'" model="Forum.moderadaoresForum" grid="col-sm-6"></config-json>', 'col-sm-12'), 
Form::getModel(Html::inputSelectNgRepeat('Forum.idUsuario', Config::getAliasesField('idUsuario'), 'Forum.idUsuario_ro', 'Aux.Usuario', $ngClick, $ngChange), 'col-sm-4')
];
...
$tableFiltros = new Table(['','',''], false, false, 'table-bordered', false);$tableFiltros->setExplode(false);$tableFiltros->addLinha([
                  '<p class="text-strong">GradePolo</p>
                  <p ng-repeat="filter in Aux.GradePolo | filter: {idGradePolo:Args.idGradePolo}:true">{{filter.nomeGradePolo}}</p>',
