...
['grid' => 'col-6 col-sm-4', 'entidade' => 'Post']];

// Campos apresentados na lista
$PostReads = [
            ['label' => Config::getAliasesField('idUsuario'), 'atributo' => 'Usuario.nomeUsuario', 'class'=>'text-left'],

            ['label' => Config::getAliasesField('idPost'), 'atributo' => 'Post.nomePost', 'class'=>'text-left']
    ];
$PostReads[0]['ngclick'] = $onclick;

// cards
$card = Card::basic($PostReads, $entidade, false, $entidade.'ContextItens');

// table
foreach ($PostReads as $campo) {
...
// Form
$form = [
Form::getModel(Html::inputSelectNgRepeat('PostRead.idUsuario', Config::getAliasesField('idUsuario'), 'PostRead.idUsuario_ro', 'Aux.Usuario', $ngClick, $ngChange), 'col-sm-4'), 
Form::getModel(Html::inputSelectNgRepeat('PostRead.idPost', Config::getAliasesField('idPost'), 'PostRead.idPost_ro', 'Aux.Post', $ngClick, $ngChange), 'col-sm-4')
];
...
$tableFiltros = new Table(['','',''], false, false, 'table-bordered', false);$tableFiltros->setExplode(false);$tableFiltros->addLinha([
                  ...
                  '<p class="text-strong">Post</p>
                  <p ng-repeat="filter in Aux.Post | filter: {idPost:Args.idPost}:true">{{filter.nomePost}}</p>',
