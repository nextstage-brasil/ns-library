...
$Trashs = [
            ['label' => Config::getAliasesField('entidadeTrash'), 'atributo' => 'entidadeTrash', 'class'=>'text-left'],

            ['label' => Config::getAliasesField('usuarioTrash'), 'atributo' => 'usuarioTrash', 'class'=>'text-left'],

            ['label' => Config::getAliasesField('jsonTrash'), 'atributo' => 'jsonTrash', 'class'=>'text-left']
    ];
$Trashs[0]['ngclick'] = $onclick;

// cards
$card = Card::basic($Trashs, $entidade, false, $entidade.'ContextItens');

// table
foreach ($Trashs as $campo) {
...
// Form
$form = [
Form::getModel(Html::input(['ng-model' => 'Trash.entidadeTrash', 'type'=>'text', 'class'=>'', 'required'=>'required ', 'ng-change' => $ngChange], Config::getAliasesField('entidadeTrash')), 'col-sm-4'), 
Form::getModel(Html::input(['ng-model' => 'Trash.usuarioTrash', 'type'=>'text', 'class'=>'', 'required'=>'required ', 'ng-change' => $ngChange], Config::getAliasesField('usuarioTrash')), 'col-sm-4'), 
Form::getModel('<config-json title="'.Config::getAliasesField('jsonTrash').'" model="Trash.jsonTrash" grid="col-sm-6"></config-json>', 'col-sm-12')
];
