...
$Auxiliars = [
            ['label' => Config::getAliasesField('nomeAuxiliar'), 'atributo' => 'nomeAuxiliar', 'class'=>'text-left'],

            ['label' => Config::getAliasesField('extrasAuxiliar'), 'atributo' => 'extrasAuxiliar', 'class'=>'text-left']
    ];
$Auxiliars[0]['ngclick'] = $onclick;

// cards
$card = Card::basic($Auxiliars, $entidade, false, $entidade.'ContextItens');

// table
foreach ($Auxiliars as $campo) {
...
// Form
$form = [
Form::getModel(Html::input(['ng-model' => 'Auxiliar.nomeAuxiliar', 'type'=>'text', 'class'=>'', 'required'=>'required ', 'ng-change' => $ngChange], Config::getAliasesField('nomeAuxiliar')), 'col-sm-4'), 
Form::getModel('<config-json title="'.Config::getAliasesField('extrasAuxiliar').'" model="Auxiliar.extrasAuxiliar" grid="col-sm-6"></config-json>', 'col-sm-12')
];
