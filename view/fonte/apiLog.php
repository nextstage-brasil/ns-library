...
$ApiLogs = [
            ['label' => Config::getAliasesField('rotaApiLog'), 'atributo' => 'rotaApiLog', 'class'=>'text-left'],

            ['label' => Config::getAliasesField('nameApiLog'), 'atributo' => 'nameApiLog', 'class'=>'text-left'],

            ['label' => Config::getAliasesField('headersApiLog'), 'atributo' => 'headersApiLog', 'class'=>'text-left'],

            ['label' => Config::getAliasesField('requestApiLog'), 'atributo' => 'requestApiLog', 'class'=>'text-left'],

            ['label' => Config::getAliasesField('responseApiLog'), 'atributo' => 'responseApiLog', 'class'=>'text-left']
    ];
$ApiLogs[0]['ngclick'] = $onclick;

// cards
$card = Card::basic($ApiLogs, $entidade, false, $entidade.'ContextItens');

// table
foreach ($ApiLogs as $campo) {
...
// Form
$form = [
Form::getModel(Html::input(['ng-model' => 'ApiLog.rotaApiLog', 'type'=>'text', 'class'=>'', 'required'=>'required ', 'ng-change' => $ngChange], Config::getAliasesField('rotaApiLog')), 'col-sm-4'), 
Form::getModel(Html::input(['ng-model' => 'ApiLog.nameApiLog', 'type'=>'text', 'class'=>'', 'required'=>'required ', 'ng-change' => $ngChange], Config::getAliasesField('nameApiLog')), 'col-sm-4'), 
Form::getModel('<config-json title="'.Config::getAliasesField('headersApiLog').'" model="ApiLog.headersApiLog" grid="col-sm-6"></config-json>', 'col-sm-12'), 
Form::getModel('<config-json title="'.Config::getAliasesField('requestApiLog').'" model="ApiLog.requestApiLog" grid="col-sm-6"></config-json>', 'col-sm-12'), 
Form::getModel('<config-json title="'.Config::getAliasesField('responseApiLog').'" model="ApiLog.responseApiLog" grid="col-sm-6"></config-json>', 'col-sm-12')
];
