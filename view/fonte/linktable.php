...
$filtros = [['grid' => 'col-6 col-sm-4', 'entidade' => 'LtRel'],
['grid' => 'col-6 col-sm-4', 'entidade' => 'Usuario'],
['grid' => 'col-6 col-sm-4', 'entidade' => 'LeftLinktable'],
['grid' => 'col-6 col-sm-4', 'entidade' => 'RightLinktable']];

// Campos apresentados na lista
$Linktables = [
            ['label' => Config::getAliasesField('idLtRel'), 'atributo' => 'LtRel.nomeLtRel', 'class'=>'text-left'],

            ['label' => Config::getAliasesField('idUsuario'), 'atributo' => 'Usuario.nomeUsuario', 'class'=>'text-left'],

            ['label' => Config::getAliasesField('idLeftLinktable'), 'atributo' => 'LeftnomeLeftLinktable', 'class'=>'text-left'],

            ['label' => Config::getAliasesField('idRightLinktable'), 'atributo' => 'RightnomeRightLinktable', 'class'=>'text-left'],

            ['label' => Config::getAliasesField('extraLinktable'), 'atributo' => 'extraLinktable', 'class'=>'text-left']
    ];
$Linktables[0]['ngclick'] = $onclick;

// cards
$card = Card::basic($Linktables, $entidade, false, $entidade.'ContextItens');

// table
foreach ($Linktables as $campo) {
...
// Form
$form = [
Form::getModel(Html::inputSelectNgRepeat('Linktable.idLtRel', Config::getAliasesField('idLtRel'), 'Linktable.idLtRel_ro', 'Aux.LtRel', $ngClick, $ngChange), 'col-sm-4'), 
Form::getModel(Html::inputSelectNgRepeat('Linktable.idUsuario', Config::getAliasesField('idUsuario'), 'Linktable.idUsuario_ro', 'Aux.Usuario', $ngClick, $ngChange), 'col-sm-4'), 
Form::getModel(Html::inputSelectNgRepeat('Linktable.idLeftLinktable', Config::getAliasesField('idLeftLinktable'), 'Linktable.idLeftLinktable_ro', 'Aux.LeftLinktable', $ngClick, $ngChange), 'col-sm-4'), 
Form::getModel(Html::inputSelectNgRepeat('Linktable.idRightLinktable', Config::getAliasesField('idRightLinktable'), 'Linktable.idRightLinktable_ro', 'Aux.RightLinktable', $ngClick, $ngChange), 'col-sm-4'), 
Form::getModel('<config-json title="'.Config::getAliasesField('extraLinktable').'" model="Linktable.extraLinktable" grid="col-sm-6"></config-json>', 'col-sm-12')
];
...
$tableFiltros = new Table(['','','','',''], false, false, 'table-bordered', false);$tableFiltros->setExplode(false);$tableFiltros->addLinha([
                  '<p class="text-strong">LtRel</p>
                  <p ng-repeat="filter in Aux.LtRel | filter: {idLtRel:Args.idLtRel}:true">{{filter.nomeLtRel}}</p>',
                  '<p class="text-strong">Usuario</p>
                  <p ng-repeat="filter in Aux.Usuario | filter: {idUsuario:Args.idUsuario}:true">{{filter.nomeUsuario}}</p>',
                  '<p class="text-strong">LeftLinktable</p>
                  <p ng-repeat="filter in Aux.LeftLinktable | filter: {idLeftLinktable:Args.idLeftLinktable}:true">{{filter.nomeLeftLinktable}}</p>',
                  '<p class="text-strong">RightLinktable</p>
                  <p ng-repeat="filter in Aux.RightLinktable | filter: {idRightLinktable:Args.idRightLinktable}:true">{{filter.nomeRightLinktable}}</p>',
