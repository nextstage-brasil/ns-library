...
$Grades[0]['ngclick'] = $onclick;

// cards
$card = Card::basic($Grades, $entidade, false, $entidade.'ContextItens');

// table
foreach ($Grades as $campo) {
...
// Form
$form = [
Form::getModel(Html::input(['ng-model' => 'Grade.nomeGrade', 'type'=>'text', 'class'=>'', 'required'=>'required ', 'ng-change' => $ngChange], Config::getAliasesField('nomeGrade')), 'col-sm-4'), 
Form::getModel(Html::inputSelectNgRepeat('Grade.idModulo', Config::getAliasesField('idModulo'), 'Grade.idModulo_ro', 'Aux.Modulo', $ngClick, $ngChange), 'col-sm-4'), 
Form::getModel(Html::inputSelectNgRepeat('Grade.idCurso', Config::getAliasesField('idCurso'), 'Grade.idCurso_ro', 'Aux.Curso', $ngClick, $ngChange), 'col-sm-4'), 
Form::getModel('<config-json title="'.Config::getAliasesField('extrasGrade').'" model="Grade.extrasGrade" grid="col-sm-6"></config-json>', 'col-sm-12')
];
