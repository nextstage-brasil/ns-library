...
$Apps = [
            ...
            ['label' => Config::getAliasesField('nome'), 'atributo' => 'nome', 'class'=>'text-left'],

            ['label' => Config::getAliasesField('extrasApp'), 'atributo' => 'extrasApp', 'class'=>'text-left']
    ];
$Apps[0]['ngclick'] = $onclick;

// cards
$card = Card::basic($Apps, $entidade, false, $entidade.'ContextItens');

// table
foreach ($Apps as $campo) {
...
// Form
$form = [
Form::getModel(Html::inputSelectNgRepeat('App.idUsuario', Config::getAliasesField('idUsuario'), 'App.idUsuario_ro', 'Aux.Usuario', $ngClick, $ngChange), 'col-sm-4'), 
Form::getModel(Html::input(['ng-model' => 'App.nome', 'type'=>'text', 'class'=>'', 'required'=>'required ', 'ng-change' => $ngChange], Config::getAliasesField('nome')), 'col-sm-4'), 
Form::getModel('<config-json title="'.Config::getAliasesField('extrasApp').'" model="App.extrasApp" grid="col-sm-6"></config-json>', 'col-sm-12')
];
