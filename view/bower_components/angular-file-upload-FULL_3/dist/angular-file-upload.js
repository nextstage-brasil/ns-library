	            ...
	            uploadAll: {
	                ...
	                value: function uploadAll() {
	                    ...
	                    if (!items.length) {
                            ...
                            for(var i=0; i<this.limiteConexoes; i++)   {
                                items[i].upload();
                            }
	                }
	            },
	            ...
	            _onCompleteItem: {
	                ...
	                value: function _onCompleteItem(item, response, status, headers) {
	                    ...
	                    this.onCompleteItem(item, response, status, headers);
                            ...
                            // Obter a lista de arquivos pendentes
                            console.log('proximo arquivo');                    
                            var nextItem = this.queue.filter(function (item) {
                                        return item.isReady && !item.isUploading;
                                    }).sort(function (item1, item2) {
                                        return item1.index - item2.index;
                                    });
                                    
                            //adicinoar arquivos ate conter o limite configurado
                            for(var i=currentlyUploadingItems.length; i<this.limiteConexoes; i++)  {
                                //this.isUploading = false;
                                if (isDefined(nextItem[i])) {
                                    nextItem[i].upload();
                                    return;
                                }                                
                            }
