...
class EntidadesCreate {
    ...
    public static function get($dados) {
        $out = '<?php
            /** CREATE AT ' . date('d/m/Y H:i:s') . ' BY NEXTSTAGE FRAMEWORK **/
...
private $table = "' . $dados['tabela'] . '";
private $cpoId = "' . $dados['cpoID'] . '";';
        $getSet[] = Helper::escreveTemplate(self::$getterSetterPadrao, array('cpoID' => $dados['cpoID']));
        foreach ($dados['atributos'] as $val) {
            ...
            $getSet[] = Helper::escreveTemplate($template, $val);
            $constructSet[] = Helper::escreveTemplate(self::$setterConstruct, $val);
        }
